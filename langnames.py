...
# Consonants
nclist = ['b', 'c', 'd', 'f', 'g', 'h', 'j', 'k', 'l', 'm', 'n', 'p', 'r', 's', 't', 'y']
...
def MakeLang(langNormPerc=False):
    ...
    if langUsesVC:
        ...
        for cluster in vcclist:
            ...
            vcweightlist.extend([cluster] * clusterweight * (
                vweightlist.count(list(cluster)[0]) + vweightlist.count(list(cluster)[1]) + random.randrange(1, 6)))
    ...
    if langUsesCC:
        ...
        for cluster in cculist:
            ...
            ccweightlist.extend([cluster] * clusterweight * int(
                (float(math.sqrt((3 - langNormPerc) * (3 - langNormPerc))) / 1.0) * 2 * random.randrange(1, 25)))
        if langRC:
        ...
        if langRSCC:
            for cluster in ccrslist:
                ...
                for letter in list(cluster):
                    clusterweight += lwlist[letter]
                clusterweight = int(clusterweight / 10)
                ccweightlist.extend([cluster] * clusterweight * (
                    langNormPerc * ((100 - langHSPerc) / random.randrange(30, 50))) * random.randrange(1, 5))
        if langRHCC:
            for cluster in ccrhlist:
                ...
                clusterweight = int(clusterweight / 10)
                ccweightlist.extend([cluster] * clusterweight * (
                    langNormPerc * ((langHSPerc) / random.randrange(30, 50))) * random.randrange(1, 5))
